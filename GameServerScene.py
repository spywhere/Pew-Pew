...
class GameServerScene(Scene):
		...
		if isinstance(packet, Packets.PlayerInfoPacket):
			if packet.getHealth() > 0:
				if client not in self.players:
					player = Player(self.getGame(), color=Color(random.randint(127, 255), random.randint(127, 255), random.randint(127, 255)))
		...
		for client in self.players:
			if client.isClientAlive():
				...
				if ppos != player.getPosition() or pangle != player.getAngle():
					self.serverSocket.sendPacket(PlayerInfoPacket(player.getPosition(), player.getVelocity(), player.getAngle(), player.getHealth(), player.getPlayerId()), client)
				client.sendPacket(PlayerGunPacket(player.getGun().isReloading(), player.getGun().getBulletInMagazine(), player.getGun().getTotalBullet()))
	...
	def onExit(self):
		...
		while self.serverSocket.isConnected() and disconnectTimeout+3.0 > time.time():
			continue
