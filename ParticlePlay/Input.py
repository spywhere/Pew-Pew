class Input:
    ...
    def isKeyDown(self, key):
        return self._keydown is not None and self._keydown.has_key(str.lower(key)) and self._keydown[str.lower(key)]
    ...
    def isMouseDown(self, button=0, cooldown=0):
        if self._isCoolDown():
            return False
        mousedown = self._mousedown[button] is not None and self._mousedown[button]
        if mousedown:
