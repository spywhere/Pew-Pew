...
class Renderer:
	...
	def __init__(self, game):
		...
		self.setPostOffset([self._game.getWidth() / 2, self._game.getHeight() / 2])

	def _isOutside(self, coords):
		for coord in coords:
			if coord[0] >= 0 and coord[1] >= 0 and coord[0] < self._game.getWidth() and coord[1] < self._game.getHeight():
				return False
		return True
	...
	def _getPosition(self, positions):
		if (isinstance(positions, list) or isinstance(positions, tuple)) and (isinstance(positions[0], int) or isinstance(positions[0], float)):
	...
	def _toOvalCoord(self, position, size):
		if len(position) == 2 and len(size) == 2:
			return [(position[0]-size[0]/2, position[1]-size[1]/2), (position[0]-size[0]/2, position[1]+size[1]/2), (position[0]+size[0]/2, position[1]+size[1]/2), (position[0]-size[0]/2, position[1]+size[1]/2)]
		else:
	...
	def _toRectCoord(self, position, size):
		...
		if len(position) == 3 and len(size) >= 2:
			...
			if len(size) == 3:
				...
				c.append((position[0]+size[0], position[1], position[2]+size[2]))
				c.append((position[0]+size[0], position[1]+size[1], position[2]+size[2]))
				c.append((position[0], position[1]+size[1], position[2]+size[2]))
			return c
		elif len(position) == 2 and len(size) == 2:
			# Rectangle
			return [(position[0], position[1]),(position[0]+size[0],position[1]),(position[0]+size[0], position[1]+size[1]),(position[0],position[1]+size[1])]
		else:
			print "Position and size must be at least 2-tuple"
			raise Exception
	...
	def setLayer(self, obj, layer):
		if layer > 0:
			self._game._mainCanvas.tag_lower(obj, layer) # Higher should be more visible
	...
	def drawLine(self, coords, cap=tk.BUTT, join=tk.ROUND, roundness=0):
		if self._isOutside(self._getPosition(coords)):
			return None
		self._game._mainCanvas.create_line(self._toCoord(self._getPosition(coords)), fill=self._fgColor, smooth=roundness, width=self._strokeSize, capstyle=cap, joinstyle=join)
	...
	def drawString(self, position, string, anchor=tk.NW, align=tk.LEFT, width=None):
		self._game._mainCanvas.create_text(position[0], position[1], fill=self._fgColor, width=width, text=string, anchor=anchor, justify=align)

	def fillArc(self, position, size, start=0, end=360, fill=None):
		if self._isOutside(self._getPosition(self._toRectCoord(position, size))):
			return None
		if fill is None:
			fill = self._fgColor
		start += 180
		end += 180
		self._game._mainCanvas.create_arc(position[0], position[1],position[0]+size[0], position[1]+size[1], outline=self._fgColor,fill=fill, width=self._strokeSize, style=tk.ARC, start=start, extent=start-end)

	def fillOval(self, position, size, fill=None):
		if self._isOutside(self._getPosition(self._toOvalCoord(position, size))):
			return None
		if fill is None:
			fill = self._fgColor
		self._game._mainCanvas.create_oval(position[0]-(size[0]/2), position[1]-(size[1]/2),position[0]+(size[0]/2), position[1]+(size[1]/2), outline=self._fgColor,fill=fill, width=self._strokeSize)
	...
	def fillPolygon(self, coords, roundness=0, fill=None):
		...
		if fill is None:
			fill = self._fgColor
		self._game._mainCanvas.create_polygon(self._toCoord(self._getPosition(coords)), outline=self._fgColor, fill=fill, smooth=roundness, width=self._strokeSize)
	...
	def addPostOffset(self, offset):
		self._render3d._postOffset = self._render3d._postOffset+offset
