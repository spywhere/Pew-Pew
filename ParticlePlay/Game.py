...
class Game:
    ...
    def startGame(self):
        ...
        try:
            self._mainCanvas = tk.Canvas(self._mainWindow, width=self._size[0], height=self._size[1], borderwidth=0)
        ...
        while self._running:
            ...
            if updateDelta > microPerUpdate:
                ...
                if self._currentScene is not None:
                    self._gameScenes[self._currentScene].onUpdate(self._gameInput, timeDelta/1000000.0)
            ...
            if not self._vSync or renderDelta > microPerRender:
                ...
                if self._currentScene is not None:
                    ...
                    self._gameScenes[self._currentScene].onRender(self._renderer, renderDelta)
                    if self._showFPS:
                        self._renderer.setColor(Color(255, 255, 255))
                        self._renderer.drawString((10, 5), self._infoText, tk.NW, tk.LEFT)

            self.sleep(self._idleTime)

            if self.getTimeMicros() - lastTimer >= 1000000:
                self._PSInfo = [frames, updates, timeDelta/1000.0]
                self._infoText = "%d FPS, %d UPS [%0.2fms]" % (frames, updates, timeDelta/1000.0)
                if self._debugMode:
    ...
    def stopGame(self):
        ...
        if self._currentScene is not None and not self._gameScenes[self._currentScene].willExit():
            return
        if self._debugMode:
                print "Closing game..."
        self._running = False
